...
if __name__ == '__main__':
    ...
    if args.db_content == 0:
        word_emb = load_word_and_type_emb('glove/glove.42B.300d.txt', "para-nmt-50m/data/paragram_sl999_czeng.txt",\
                                            val_sql_data, val_table_data, args.db_content, is_list=True, use_htype=False, use_small = USE_SMALL)
    else:
        word_emb = load_concat_wemb('glove/glove.42B.300d.txt', "para-nmt-50m/data/paragram_sl999_czeng.txt", use_small=USE_SMALL)
    ...
    best_cond_idx = 0
    print('Init dev acc_qm:\n  breakdown on (agg, sel, where): ' , init_acc)
    if TRAIN_AGG:
